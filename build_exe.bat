...
echo.
echo === Framework Brief Desk : build EXE ===
echo.
...
taskkill /F /IM FrameworkBriefDesk.exe >nul 2>&1
timeout /t 2 /nobreak >nul

if exist "dist\FrameworkBriefDesk.exe" (
  del /f /q "dist\FrameworkBriefDesk.exe" >nul 2>&1
)
if exist "dist\FrameworkBriefDesk.exe" (
  echo [ERROR] Cannot replace dist\FrameworkBriefDesk.exe
...
echo Building EXE ...
"%VPY%" -m PyInstaller --noconfirm --clean FrameworkBriefDesk.spec
if errorlevel 1 (
...
echo.
echo OK
echo EXE: "%cd%\dist\FrameworkBriefDesk.exe"
echo.
if exist "dist\FrameworkBriefDesk.exe" explorer dist
pause
